...
def out_of_mem_or_timeout(d):
	for key, value in d.items():
		length = len(value)
		if (length != runTimes):
			print(str(runTimes-length) + "/" + str(runTimes) + " of " + key + " run out of memory or timeout by using flowdroid")
...
def analysis_flowdroid():
	...
	for app,value in app2abbr.items():
...
def method_source():
	...
	for i in range(runTimes):
		run_disk(group1Path, 'ms.jar', i, msDir)
		run_disk(group2Path, 'ms.jar', i, msDir)
		run_disk(group3Path, 'ms.jar', i, msDir)
		run_disk(group4Path, 'ms.jar', i, msDir)
	for i in range(runTimes):
...
def method_target():
	...
	for i in range(runTimes):
		run_disk(group1Path, 'mt.jar', i, mtDir)
		run_disk(group2Path, 'mt.jar', i, mtDir)
		run_disk(group3Path, 'mt.jar', i, mtDir)
		run_disk(group4Path, 'mt.jar', i, mtDir)
	for i in range(runTimes):
...
def source():
	...
	for i in range(runTimes):
		run_disk(group1Path, 's.jar', i, sDir)
		run_disk(group2Path, 's.jar', i, sDir)
		run_disk(group3Path, 's.jar', i, sDir)
		run_disk(group4Path, 's.jar', i, sDir)
	for i in range(runTimes):
...
def target():
	...
	for i in range(runTimes):
		run_disk(group1Path, 't.jar', i, tDir)
		run_disk(group2Path, 't.jar', i, tDir)
		run_disk(group3Path, 't.jar', i, tDir)
		run_disk(group4Path, 't.jar', i, tDir)
	for i in range(runTimes):
...
def write_random_50():
	...
	for i in range(runTimes):
		run_disk(group1Path, 's-random.jar', i, writeRandom50Dir)
		run_disk(group2Path, 's-random.jar', i, writeRandom50Dir)
		run_disk(group3Path, 's-random.jar', i, writeRandom50Dir)
		run_disk(group4Path, 's-random.jar', i, writeRandom50Dir)
	for i in range(runTimes):
...
def write_70():
	...
	for i in range(runTimes):
		run_disk(group1Path, 's-write70.jar', i, write70Dir)
		run_disk(group2Path, 's-write70.jar', i, write70Dir)
		run_disk(group3Path, 's-write70.jar', i, write70Dir)
		run_disk(group4Path, 's-write70.jar', i, write70Dir)
	for i in range(runTimes):
...
def write_worklist():
	...
	for i in range(runTimes):
		run_disk(group1Path, 's-onlyWorklist.jar', i, writeWorklistDir)
		run_disk(group2Path, 's-onlyWorklist.jar', i, writeWorklistDir)
		run_disk(group3Path, 's-onlyWorklist.jar', i, writeWorklistDir)
		run_disk(group4Path, 's-onlyWorklist.jar', i, writeWorklistDir)
	for i in range(runTimes):
...
# Distribution of pass edge access number for CGAB
def edge_access_num():
	print("\r\n")
	print("Figure 4: Distribution of pass edge access number for CGAB")
	cmd = "java -noverify -Xmx60g -Xms60g -jar lib/PECount.jar -a " + group2Path + "/com.github.axet.bookreader_375.apk -s SourcesAndSinks.txt -t EasyTaintWrapperSource.txt -p platforms -dt 10800 > result/CGABcount 2>&1"
	#print(cmd)
#	os.system(cmd)
	pathedgeCount.distribution("result/CGABcount")
	
def calculate_avg(originDict, result):
	for key, value in originDict.items():
		result[key] = round(np.mean(value))

def run_disk(targetDir, jarFile, times, outputParent):
	for parent, dirnames, apps in os.walk(targetDir, followlinks=True):
		for app in apps:
			appPath = os.path.join(parent, app)
...
def run_flowdroid(times, jarFile, outputParent):
	for parent, dirnames, apps in os.walk(group1Path, followlinks=True):
		for app in apps:
			appPath = os.path.join(parent, app)
			outputDir = os.path.join(outputParent, str(times))
			if not os.path.isdir(outputDir):
	...
	for parent, dirnames, apps in os.walk(group2Path, followlinks=True):
		for app in apps:
			appPath = os.path.join(parent, app)
			outputDir = os.path.join(outputParent, str(times))
			if not os.path.isdir(outputDir):
	...
	for parent, dirnames, apps in os.walk(group3Path, followlinks=True):
		for app in apps:
			appPath = os.path.join(parent, app)
			outputDir = os.path.join(outputParent, str(times))
			if not os.path.isdir(outputDir):
	...
	for parent, dirnames, apps in os.walk(group4Path, followlinks=True):
		for app in apps:
			appPath = os.path.join(parent, app)
			outputDir = os.path.join(outputParent, str(times))
			if not os.path.isdir(outputDir):
...
def run_flowdroid_60(targetDir, times, jarFile, outputParent):
	for parent, dirnames, apps in os.walk(targetDir, followlinks=True):
		for app in apps:
			appPath = os.path.join(parent, app)
			outputDir = os.path.join(outputParent, str(times))
...
def mem_ratio():
	...
	run_flowdroid_60(group1Path, 0, 'memRatio.jar', memRatioDir)
	run_flowdroid_60(group2Path, 0, 'memRatio.jar', memRatioDir)
	run_flowdroid_60(group3Path, 0, 'memRatio.jar', memRatioDir)
	run_flowdroid_60(group4Path, 0, 'memRatio.jar', memRatioDir)
...
if __name__ == '__main__':
	...
	parser.add_argument('-times', type=int, default=5, help='Analaysis times for each benchmark')
	...
	group1Path = os.path.join(rootDir, "group1")
	group2Path = os.path.join(rootDir, "group2")
	group3Path = os.path.join(rootDir, "group3")
	group4Path = os.path.join(rootDir, "group4")
	...
	if args.k == 'ALL':
		...
		source()
		table = PrettyTable(['Abbr', 'Time(s)'])
		for app,value in app2abbr.items():
			dt = sTimeAvg[app] if app in sTimeAvg else "NULL"
			table.add_row([app2abbr[app], dt])
		print(table)
